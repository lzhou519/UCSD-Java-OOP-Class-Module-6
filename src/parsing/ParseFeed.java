...
public class ParseFeed {
	...
	public static HashMap<String, Float> loadGDPperCapitaFromCSV(PApplet p, String fileName) {
		// HashMap key: country ID and  data: lifeExp at birth
		HashMap<String, Float> gdpPerCapitaMap = new HashMap<String, Float>();

		// get lines of csv file
		String[] rows = p.loadStrings(fileName);

		// Reads country name and GDP per Capita value from CSV row
		for (String row : rows) {
			// split row by commas not in quotations
			String[] columns = row.split(",(?=([^\"]*\"[^\"]*\")*[^\"]*$)");
			...
			for(int i = columns.length - 1; i > 3; i--) {
				...
				// check if value exists for year
				if(!columns[i].equals("..")) {
					gdpPerCapitaMap.put(columns[1], Float.parseFloat(columns[i]));
